...
PNG createSpotlight(PNG image, int centerX, int centerY) {
	...
	for (unsigned x = 0; x < image.width(); x++) {
		for (unsigned y = 0; y < image.height();  y++) {
			...
			int dist_squared = x_dist*x_dist + y_dist*y_dist;
			double lum_factor = 1 - (sqrt(dist_squared) / 200);
			if (lum_factor < 0) {
				lum_factor = 0;
			}
...
PNG watermark(PNG firstImage, PNG secondImage) {
    ...
    unsigned width1 = firstImage.width();
    unsigned width2 = secondImage.width();
    unsigned height1 = firstImage.height();
    unsigned height2 = secondImage.height();

    firstImage.resize(min(width1, width2), min(height1, height2));
    secondImage.resize(min(width1, width2), min(height1, height2));
