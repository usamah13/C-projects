...
class BinaryTree
{
  ...
  private:
    ...
    /**
     * private helper for printing in-Order traversal
     * @param start_node node to start printing
     * @return none
     */
    void printLeftToRight(const Node* start_node) const;

    void mirror(Node *& start_node);

    void printPaths(const Node *start_node, string toPrint) const;

    int sumDistances(const Node* start_node, int dist) const;
