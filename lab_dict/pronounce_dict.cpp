...
bool PronounceDict::homophones(const string& word1, const string& word2) const
{
    ...
    std::transform(copy_1.begin(), copy_1.end(), copy_1.begin(), ::toupper);
    std::transform(copy_2.begin(), copy_2.end(), copy_2.begin(), ::toupper);

    auto lookup_1 = dict.find(copy_1);
    auto lookup_2 = dict.find(copy_2);

    if (lookup_1 == dict.end() || lookup_2 == dict.end()){
        return false; // not in dictionary
    }

    if (lookup_1->second == lookup_2->second) return true;
