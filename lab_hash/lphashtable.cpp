...
void LPHashTable<K, V>::insert(K const& key, V const& value)
{
	...
	// int i = 0; // probing step
	 
	 /*if (table[index % size] == NULL){
		 
		 table[index % size] = new pair<K, V>(key, value); // need to dynamically allocate?
		 should_probe[index % size] = true;
	 } else {
		 while(table[(index + i) % size] != NULL){
			 i++; // move forward by 1 step
		 }
		 
		 table[(index + i) % size] = new pair<K, V>(key, value); // need to dynamically allocate?
		 should_probe[(index + i) % size] = true;
	 }*/
